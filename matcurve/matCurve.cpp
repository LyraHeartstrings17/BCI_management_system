    ...
    ui(new Ui::MatCurve)
{
    ...
    connect(ui->pushButton_7, &QPushButton::clicked, this, &MatCurve::ShowButtonClicked);
...
//        ui->widget->appendMark(mati.readEventtype(this->path),mati.readEventlatency(this->path)); // 向主窗口添加标记（根据具体实现，可能是用于在曲线图中添加特定的标记）
//        ui->widget->appendMark("123");
        //app.exec(); // 运行应用程序的事件循环，等待事件的发生并响应
        vector<unsigned int> eventtype=mati.readEventtype(this->path);
        vector<unsigned int> lantency=mati.readEventlatency(this->path);
        QString type;
        ...
        for(int i=0;i<length;i++)
        {
            ...
            ui->widget->appendMark(type,lantency[i]); // 向主窗口添加标记（根据具体实现，可能是用于在曲线图中添加特定的标记）
        }
    }
...
void MatCurve::ShowButtonClicked()
{
    ...
    if(ui->widget->getTimerId()==0)//第一次点击开始
    {
        ...
        ui->widget->loadData();
        QStringList list = ui->widget->getChanlocs();
        ui->widget->setMaxPoint(20000); // 设置曲线图显示的最大点数为20000个
        ...
        ui->widget->test_chart2(); // 进行曲线图的测试（根据具体实现，可能是用于生成测试数据）
